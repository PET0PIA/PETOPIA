-- =========================================================
-- V48__chat_rework.sql
-- 고정형 답변을 상담 세션에서 떼어내고, 상담 세션을 "상담원 연결" 하나로 한정한다.
-- =========================================================
--
-- 번호가 46이 아니라 48인 이유: 이 파일이 PR로 열려 있는 동안 다른 브랜치가 V47을 먼저
-- 머지·배포했다. 운영 DB는 47까지 적용된 상태에서 46을 받았고, out-of-order: false인
-- Flyway는 이를 "순서를 거스른 마이그레이션"으로 보고 기동 자체를 막았다. 앱이 뜨지 못해
-- ALB가 계속 옛 컨테이너로 트래픽을 보냈고, 프론트만 새 버전이 되어 고정 답변 말풍선이
-- 비고 클릭 집계가 404로 떨어졌다.
--
-- out-of-order를 켜는 대신 번호를 올린다. 그 플래그는 지금 같은 사고를 막으려고 의도적으로
-- 꺼둔 값이고(application.yaml), 켜면 다음부터 같은 상황이 조용히 지나간다.
--
...
--
-- 값까지 FIXED로 바꾼다. 내리는 것만으로는 부족하다 - answer_type에 'AI'가 남으면
-- ChatAnswerType(FIXED, AGENT)에 없는 값이 되고, 관리자 목록 조회(selectAllMenus)는
-- 비활성 행까지 읽으므로 MyBatis 기본 enum 핸들러가 Enum.valueOf에서 던진다. 버튼 하나 때문에
-- 상담 설정 화면 전체가 500이 된다. 실제로 운영의 VENDOR_INQUIRY가 AI로 바뀌어 있었다.
--
-- FIXED로 되돌리는 이유는 그 행의 출신이 FIXED(V28 시딩)이고, FIXED는 상담을 만들지 않아
-- 값을 되돌려도 부작용이 없기 때문이다. 답변 본문이 비어 있을 수 있지만 is_active = 0이라
-- 위젯에는 나오지 않고, 관리자 화면이 "답변이 비어 있으면 눌러도 아무 내용이 보이지 않아요"로
-- 경고한다 - 운영자가 본문을 채운 뒤 다시 올리면 된다.
