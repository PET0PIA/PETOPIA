...
export function HeroSection() {
  ...
  // 로딩 중에도 실제와 같은 구조를 그린 뒤 invisible로 숨겨서, 데이터가 도착했을 때 레이아웃이
  // 출렁이지 않게 한다. 자리 높이를 숫자로 박아두면 배너 크기를 조정할 때마다 실제와 어긋난다.
  // 노출 중인 배너가 0개로 확정되면 광고 문의로 이어지는 기본 배너를 대신 보여준다 - 완전히
  // 빈 화면보다는 광고 영업 기회로 쓰는 편이 낫다.
  const loading = banners === null;
  ...
  return (
    <section className={`relative overflow-hidden transition-colors duration-500 motion-reduce:transition-none${loading ? " invisible" : ""}`} style={{ backgroundColor: slide.bgColor ?? "var(--color-point-yellow)" }} aria-hidden={loading || undefined}>
      ...
      <div className="page-shell pointer-events-none relative z-10 grid items-center gap-8 pt-10 sm:pt-14 md:grid-cols-[0.75fr_1.25fr]">
        <div key={loading ? "skeleton" : index} className="animate-[heroTextIn_600ms_ease-out_both] py-3 motion-reduce:animate-none">
        ...
        <div className="relative mx-auto flex h-56 w-full items-center justify-center sm:h-64 md:h-[20rem] md:justify-end lg:h-[24rem]">
          {!loading && <img key={index} src={slide.imageKey} alt="" className="max-h-full max-w-full animate-[heroPosterIn_600ms_ease-out_both] object-contain motion-reduce:animate-none" />}
        </div>
      </div>
      {slides.length > 1 && (
