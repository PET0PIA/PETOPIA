...
/**
 * 행사 상세의 "참가기업" 섹션. confirmed-booths(공개)로 참가기업 수를 세고, 배치도(PublicBoothLayoutCanvas)를 보여준다.
 * 카드 목록은 별도 페이지(FairBoothsPage, /fairs/:fairId/booths)로 분리했다 - 진입은
 * FairDetailPage의 "부스 목록" 버튼에서 한다. 배치도의 슬롯 클릭 시 뜨는 업체 정보 패널은
...
export function FairParticipatingBooths({ fairId }: { fairId: number }) {
  ...
  return (
    <section className="mt-10">
      ...
      {/* 배치도(읽기 전용) - 부스를 클릭하면 업체 정보 패널이 뜬다. 카드 목록과 같은
