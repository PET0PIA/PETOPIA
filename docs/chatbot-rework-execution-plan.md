...
~~현재 `src/test`에는 상담 관련 테스트가 하나도 없다.~~ **작성됨** — 40건(`./gradlew test
--tests "com.ms.petopia.api.chat.*"` 실행 건수).

| 클래스 | 건수 | 무엇을 지키는가 |
| --- | --- | --- |
| `ChatConversationServiceTest` | 12 | 고정형이 상담을 못 만드는가, 클릭이 세션을 안 만드는가, AI 예약 조건 |
| `ChatAiAnswerServiceTest` | 7 | 마무리 안내가 한 말풍선인가, 전이 실패 시 아무것도 저장하지 않는가, 이관 안내의 상태 게이트와 중복 |
...
### 12-2. 2차 리뷰 (같은 날, 커밋 `32497ea`)

위 반영 커밋에 증분 리뷰가 7건 더 붙었다. 클릭 집계 지적은 근거를 답글로 남긴 뒤 리뷰어가
철회했고(스레드 종료), 나머지는 아래처럼 갈랐다.

| 지적 | 원인 | 처리 |
| --- | --- | --- |
| 전송 중 `상담 종료`가 눌린다. 서버가 종료를 먼저 처리하면 방금 쓴 메시지가 사라진다 (**Major**) | 기존 | `closing` 상태를 따로 두고 두 동작이 서로를 막게 했다. 버튼도 잠그지만 판정은 핸들러에서 한다 - 연달아 눌리는 경로가 화면 상태보다 빠르다 |
| 이관 안내가 상태를 보지 않는다. 종료된·인수된 대화에도 "상담사가 확인 후 답변드릴게요"가 붙는다 | 기존 | 행을 잠근 뒤 상태를 다시 읽고 `needsAgentReply()`일 때만 붙인다. 답변 경로에 넣은 가드와 같은 규칙이다 |
| `refresh`에 겹침 방어가 없다. 늦게 온 옛 응답이 새 상태를 덮고, 답변·종료 직후 호출은 취소 플래그도 없다 | 12-1의 변경 | 갱신 세대 카운터. 이기는 쪽을 "가장 마지막에 시작한 갱신"으로 못박았다 |
| 건수 조회가 첫 성공 전에 실패하면 머리말이 "불러오고 있어요"로 굳는다 | 12-1의 변경 | 실패를 `waitingCountFailed`로 따로 들었다. 조용히 넘기는 것과 로딩으로 위장하는 것은 다르다 |
| 연결 요청 중 뒤로가기가 살아 있어 MENU로 돌아가도 THREAD로 끌려간다 | 기존 | `disabled={busy}` |
| `문의 내역`으로 넘어갈 때 이전 화면의 오류가 남는다 | 기존 | `goThread()`로 묶어 화면 전환과 오류 정리를 한 자리에 뒀다 |
| 진행 중 상담 재사용 경로에서 AGENT 클릭이 집계되지 않는다 | 기존 | **반영 안 함.** 방금 리뷰어가 받아들인 지표 정의가 "AGENT의 클릭 수 − 접수 수 = 누르고 그만둔 비율"이다. 돌아가기 클릭까지 세면 그 분모가 오염돼 지표가 무의미해진다. 재방문 횟수가 필요해지면 클릭이 아니라 별도 이벤트로 센다 |
