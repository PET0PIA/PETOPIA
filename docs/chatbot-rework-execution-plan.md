...
원하는 답변이 아니었나요?
  [ 상담원에게 직접 문의하기 ]
  ● 지금은 상담 가능해요            ← 운영시간 안
  ○ 자동 응대로 먼저 답변드려요      ← 운영시간 밖 (둘 중 하나만 보인다)
```
...
| `components/chat/FixedAnswerView.tsx` | 신규. 답변 본문 + 상담원 연결 CTA(6-1-1). 연결 대상 판정과 진행 중 상담 분기는 `ChatWidget`이 갖고 콜백만 받는다 |
...
현재 `src/test`에는 상담 관련 테스트가 **하나도 없다**(`api/chat` 디렉터리 자체가 없다).
아래 세 클래스는 모두 신규 작성이며, 그만큼 8번 단계의 분량을 실제보다 작게 잡지 않도록 한다.
